...
- **Supabase**: project ref `mrwrmigeyatfrzwvfsfe` (MCP server configured, schema created — 19 tables, 4 views, 3 RPCs, seed data)

## Stack
Flutter 3.x + Dart, Riverpod, GoRouter, Freezed. Supabase connected (auth + user profiles live).
...
- [x] Opportunities screen (business model tabs + location filter, project list)
...
- [x] Auth flow (welcome carousel + login + register + Supabase Auth + router guard + profile wired)
- [ ] Repository interfaces (abstract + mock impl)
- [x] Supabase schema created (19 tables, CTI investments, views, RLS, seed data — 10 migrations applied)
- [ ] Connect Supabase (repository interfaces + swap mock → Supabase implementations)
