...
- **News hero — multi-image carousel (ADR-70)**: when `news.video_url IS NULL` and `news.imageUrls.length > 1`, the hero renders `NewsHeroCarousel` (`features/home/presentation/widgets/news_hero_carousel.dart`) — a `PageView.builder` with native swipe + a minimal dots indicator centered on the bottom safe area (6pt diameter, 8pt gap, active alpha 1.0, inactive 0.4; tinted via `useLightOverlay`). Dots fade out via `AnimatedOpacity` ligada a `_heroGone` so they disappear cuando el SliverAppBar colapsa. Only slot 0 owns the `Hero(tag: 'news-hero-<id>')` — the shared-element flight from feed/card always lands on the first image. Subsequent slots are plain `LhotseImage`. The same top/bottom gradients (top: `Color(0x66000000)→transparent`, bottom: `transparent→Color(0x8C1F1916)`) sit above every slot. **Video wins**: if `video_url` is set the carrusel desaparece, el hero vuelve al single-frame poster con `LhotsePlayButton` (per ADR-62). Cards (feed, archive, L3) siguen mostrando una sola portada (`news.imageUrl`) — el carrusel vive solo en el hero del detalle.
