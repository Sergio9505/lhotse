...
- **Multi-image hero carousel (ADR-70 + ADR-71)** — news y projects comparten widget `MediaHeroCarousel` (`lib/core/widgets/media_hero_carousel.dart`). Activación: `videoUrl == null && imageUrls.length > 1` → `PageView.builder` con swipe nativo + dots indicator centrado en bottom safe area (6pt diámetro, 8pt gap, active alpha 1.0, inactive 0.4, tintado por `useLightOverlay`). Dots desaparecen via `AnimatedOpacity` ligada a `heroGone` cuando colapsa el SliverAppBar. Solo el slot 0 lleva `Hero(tag: heroTag)` — el flight desde feed/card siempre aterriza en la primera imagen; los slots 1..n son `LhotseImage` planos. Gradientes top/bottom (top `Color(0x66000000)→transparent`, bottom `transparent→Color(0x8C1F1916)`) viven encima de todo slot. **Video wins** (ADR-62 + ADR-71): si `videoUrl` está presente el carrusel desaparece y el hero muestra (a) `videoChild` inyectado por el caller cuando aplica — projects pasa `LhotseVideoPlayer` autoplay muteado, o (b) `LhotseImage.poster` + `LhotsePlayButton` cuando `videoChild == null` — news vía ADR-62. Cards (feed, archive, L1/L2/L3) siguen mostrando una sola portada (`imageUrl`) — el carrusel vive solo en el hero del detalle.
