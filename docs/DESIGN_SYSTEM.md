...
- [x] News card — `LhotseNewsCard` in `core/widgets/lhotse_news_card.dart`. Two variants:
  - **Default full**: **minimal-luxury-modern captioned photograph** (same territory as `ProjectShowcaseCard`). Edge-to-edge **1:1 image** wrapped in `Hero(tag: 'news-hero-{id}')` for shared-element transition. Caption (no hairlines): kicker `PROYECTO`/`PRENSA` (caption 10px w500 ls 2.0 accentMuted) + title `displayHero` Campton Light 48pt mixed case + deck `subtitle` bodyMedium **italic** accentMuted h:1.6 (maxLines 2 / 3 if lead) + 24pt spacer + byline `POR {BRAND} · {DATE}` (caption uppercase ls1.5) — news keeps textual byline because brand is editorial author, not maison. Required `heroTag` param. Separator between cards 56pt whitespace. Used in AllNews + NewsArchiveBody
...
- **Image overlay chips** (pattern, not a single widget) — two styles used on top of the hero image of `ProjectShowcaseCard` to communicate status without adding byline text:
  - **Fill chip (privileged)**: `AppColors.primary` bg + white caption w500 ls1.5. Used for VIP "PRIVATE" top-right.
  - **Outline chip (subordinated)**: transparent bg + 0.5px white border + soft `BoxShadow(Color(0x33000000), 8)` for legibility + white caption w500 ls1.5. Used for project phase ("EN DESARROLLO" / "FINALIZADO" / "EN FINANCIACIÓN") top-left.
  - When both coexist on the same image, the fill/outline contrast creates automatic visual hierarchy: VIP privilege dominates, phase supports
