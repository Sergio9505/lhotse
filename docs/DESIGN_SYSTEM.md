...
- [x] Project showcase card — `ProjectShowcaseCard` in `home/presentation/widgets/project_showcase_card.dart`. **Escaparate curado** (Sotheby's / Hermès catalog density). **3:2 image** `AspectRatio(3/2)` with `LhotseImage` wrapped in `Hero(tag: 'project-hero-{id}')`. Image hosts only the **VIP "PRIVATE"** fill chip top-right (gating info, not classification — see chip pattern below). Caption: title `editorialTitle` 36pt mixed case (maxLines 2) → deck `annotation` accentMuted **italic** h1.6 (maxLines 1) → 3-token byline `{BRAND uppercase} · {city} · {phase.label}` (`labelUppercaseSm` ls 1.5 for brand, `annotation` accentMuted for city+phase) → separator `  ·  ` `textPrimary` 40% alpha. `isLocked` → `showVipLockSheet`. Separator between cards `AppSpacing.lg`. Used in `ProjectsArchiveBody` (Firmas › PROYECTOS sub-tab).
...
- Both use `ScrollAwareFilterBar` and share **escaparate curado** grammar: **3:2 image**, `editorialTitle` 36pt, 1-line italic deck, byline below caption — `ProjectShowcaseCard` for projects, `LhotseNewsCard` default for news.
