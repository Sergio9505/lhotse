...
|-------|------|--------|-------|
| displayHero | 48 | Light (300) | Archive card + detail hero titles (Project + News). Line-height 0.95, letter-spacing -0.5. Minimal-luxury-modern treatment (Céline / Jil Sander / Totême territory). Use ONLY in archive showcase cards and their detail heroes |
...
- [x] Project card — `ProjectCard` in `home/presentation/widgets/project_card.dart`. Zara-style: image pure (no overlay) + text below on beige. Name at headingLarge (24px), brand·location caption below. Used in Home carousel + Opportunities. **Not** used in AllProjects/Search catálogo — see ProjectShowcaseCard below
- [x] Feed card — `FeedCard` in `home/presentation/widgets/feed_card.dart`. Universal SNKRS-inspired card for Home feed (one per viewport, 65% media + 35% beige caption). Title `displayLarge` Campton Light w300 40pt (tight line-height 1.0) — one step below archive's `displayHero` 48pt to keep Home louder than archive while sharing the same Campton Light tipographic family. Meta row (brand · location · date) bodySmall accentMuted letter-spacing 1.2, inline with right-aligned textual CTA (VER PROYECTO / LEER / VER OPORTUNIDAD / EXPLORAR) in labelLarge + arrow icon. Media block wrapped in `Hero(tag: 'project-hero-{id}' | 'news-hero-{id}')` for shared-element transition into the detail — matching the tags used in `ProjectShowcaseCard` and `LhotseNewsCard`. Supports image or muted-autoplay video via `_Media` child. Renders 4 variants via `_FeedContent` resolver: FeedProjectItem, FeedOpportunityItem, FeedNewsItem, FeedBrandItem.
- [x] Project showcase card — `ProjectShowcaseCard` in `home/presentation/widgets/project_showcase_card.dart`. **Minimal-luxury-modern premium** (Céline / Jil Sander / Totême territory, Campton-only). Edge-to-edge **1:1 square image** with `LhotseImage` wrapped in `Hero(tag: 'project-hero-{id}')` for shared-element transition into detail. Image hosts two chips: **phase chip** (`project.phase.label`) top-left as **outline** style (transparent bg + 0.5px white border + soft shadow + white caption w500 ls 1.5) and **VIP "PRIVATE"** top-right as fill black chip. Outline + fill gives automatic visual hierarchy when both coexist. Caption (no hairlines): title `displayHero` Campton Light 48pt mixed case (tight line-height 0.95) → `project.city` only (not compound location — avoids ISO codes like "Dubai, AE") in bodyMedium accentMuted mixed case → tagline bodyMedium accentMuted **italic** (maxLines 1 / 3 if lead) → 24pt spacer → `_BrandStamp` byline (SVG logo **72×20** monochrome via `ColorFilter srcIn`, same pattern as `_BrandCard` in Firmas; text fallback if `brandLogoAsset` absent). `isLocked` → `showVipLockSheet`. Separator between cards 16pt. Used in AllProjects + Search catálogo. Deferred polish: grain texture 2% overlay + parallax 0.85 on scroll
...
- [x] Strategy screen — InvestmentsScreen (full-beige collapsing hero via `SliverPersistentHeader` + `_HeroDelegate` on `AppColors.background` — no photo, no gradient, all text `textPrimary`: mixed-case title `Mi estrategia\npatrimonial` displayLarge Campton Light w300 fades out across first tramo of scroll, patrimonio total interpolates 48pt bottom-left → 28pt chrome-band center, logo+bell pinned at top; hairline separator; brand rows: name + "amount€ · %" inline left + chevron right; return % at bodySmall 12px w600 accentMuted; asterisk on estimated returns with footnote; brand marker — SVG `icon_asset` or initials monogram (RF, L&B) — no business-model breakdown, no opportunities section)
