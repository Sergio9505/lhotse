...
**Hero L2 — `titleHeight` medido en runtime con `TextPainter`.** A diferencia de L1 (título estático "Mi estrategia\npatrimonial", siempre 2 líneas), el título de L2 es dinámico (`'Mis inversiones\nen $brandName'`) y puede crecer a 3 líneas para nombres de marca largos como "Lacomb & Bos" — el `\n` rompe entre "inversiones" y "en {brand}", y luego el wrap natural divide "en Lacomb & Bos" en 2 líneas más. Calcular `titleHeight` como `scaler.scale(72)` asume 2 líneas y reserva menos espacio del que el `Text` realmente pinta → el monto se solapa con la tercera línea. La regla: en `BrandInvestmentsScreen.build`, construir un `TextPainter` con el mismo `style`, `textScaler` y `maxWidth = screenWidth - AppSpacing.lg * 2`, leer `painter.height`, y propagarlo al `_BrandHeroDelegate` por constructor (single source of truth — el delegate ya no recalcula). El `\n` se mantiene siempre para garantizar ≥ 2 líneas en marcas cortas (consistencia editorial L1↔L2).
...
- [x] Detail sub-nav (L3 tab bar) — `LhotseTabBarDelegate` in `core/widgets/lhotse_tab_bar_delegate.dart`. Pinned `SliverPersistentHeader` rendering `LhotseFilterTab(editorial: true)` items driven by a `TabController`, scrollable horizontally when overflow occurs. Editorial-luxe pattern (Hermès / Sotheby's auction lot / Apple News): content-width left-aligned tabs, underline matches each tab's text width, Title Case labels (`Avance`, `Activo`, `Finanzas`, `Docs`) — uppercase tracked stays reserved for section headers. Geometry: padding lateral `AppSpacing.lg` (24pt) **igual al inter-tab spacing `AppSpacing.lg`** (24pt) — la simetría es la regla: cuando el scroll llega al último tab (a Dynamic Type alto) o cuando los tabs caben sin scroll (textScale 1.0), el último tab queda a 24pt del edge, mismo aire que respeta entre tabs. Inter-tab spacing > padding lateral rompe la cadencia (sensación de "pegado al edge"). Shared by `coinversion_detail_screen.dart`, `direct_purchase_detail_screen.dart`, `completed_detail_screen.dart`.
...
**Brand metaphor**: Lhotse is the 4th highest mountain in the world. The intro video is the visual embodiment of the brand narrative (investing = reaching the summit). The previous CustomPainter draw animation was replaced by a pre-rendered MP4 delivered by the client — see ADR-67, which supersedes ADR-57.

- **Asset**: `assets/videos/intro_lhotse.mp4` (1080×1920, H.264/AAC, 8 s, 25 fps, ~1.5 MB).
- **Background**: pure black. Native splash (`flutter_native_splash` config in `pubspec.yaml` → `lhotse_splash.png`) is a black image that covers the ~100 ms before the first video frame is ready. Frame 0 of the video is also black, so there is no visible pop between the two.
- **Playback**: muted, no loop, `BoxFit.cover` full-bleed via `FittedBox` wrapping a sized `VideoPlayer` (the standard `BoxFit.cover` path on `VideoPlayer` does not always fill on 9:19.5 phones).
- **Duration**: ~8.5 s total — 8 s video + 0.5 s fade-out before `context.go`. Routing happens when `position >= duration` (no fixed timer).
- **Fallback**: if `VideoPlayerController.initialize()` throws (asset corrupt, codec unsupported), the screen skips the fade and navigates immediately so a broken intro never blocks boot.
- **Routing**: identical to ADR-63 — `welcome` (no session), `home` (session with `phoneConfirmedAt`), `otp-verify` with `isResume: true` (session + pending `phone_change`), or `complete-phone` (session, no phone at all).
- **Provider warm-up**: brands / projects / news / assets / document categories run in parallel during the 8 s video (authed sessions also warm portfolio + contracts). Never blocks the video timing.
- **Implementation**: `lib/features/auth/presentation/splash_screen.dart`. No haptic, no wordmark, no CustomPainter — the video carries the entire narrative.
