...
- [x] Project showcase card — `ProjectShowcaseCard` in `home/presentation/widgets/project_showcase_card.dart`. **Minimal-luxury-modern premium** (Céline / Jil Sander / Totême territory, Campton-only). Edge-to-edge **1:1 square image** wrapped in `Hero(tag: 'project-hero-{id}')` for shared-element transition into detail. Caption framed by editorial hairlines top/bottom (0.5px alpha 15%). Content order: title `displayHero` Campton Light 48pt mixed case (tight line-height 0.95) → location subtitle bodyMedium accentMuted (mixed case, no kicker) → tagline bodyMedium accentMuted **italic** (maxLines 1 / 3 if lead) → `_BrandStamp` byline (SVG logo 100×28 monochrome via `ColorFilter srcIn`, same pattern as `_BrandCard` in Firmas, reduced from 40→28pt; text fallback if `brandLogoAsset` absent) + `·` + `project.phase.label` caption uppercase. VIP "PRIVATE" chip top-right on image. `isLocked` → `showVipLockSheet`. Separator between cards 16pt. Used in AllProjects + Search catálogo. Deferred refinements (not yet implemented): grain texture 2% overlay on caption, parallax 0.85 on scroll — both low-impact polish for later iteration
...
- [x] News card — `LhotseNewsCard` in `core/widgets/lhotse_news_card.dart`. Two variants:
  - **Default full**: **minimal-luxury-modern captioned photograph** (same territory as `ProjectShowcaseCard`). Edge-to-edge **1:1 image** wrapped in `Hero(tag: 'news-hero-{id}')` for shared-element transition. Caption framed by editorial hairlines top/bottom (0.5px alpha 15%). Content: kicker `PROYECTO`/`PRENSA` (caption 10px w500 ls 2.0 accentMuted) + title `displayHero` Campton Light 48pt mixed case + deck `subtitle` bodyMedium **italic** accentMuted h:1.6 (maxLines 2 / 3 if lead) + byline `POR {BRAND} · {DATE}` (caption uppercase ls1.5) — news keeps textual byline because brand is editorial author, not maison. Required `heroTag` param. Separator between cards 56pt whitespace. Used in AllNews + NewsArchiveBody
