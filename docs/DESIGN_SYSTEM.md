...
- [x] Brand filter row — `LhotseBrandFilterRow` in `core/widgets/lhotse_brand_filter_row.dart`. Horizontal scroll of `BrandWordmark(xs)` (24pt height) — fallback a primera inicial — con multi-select por opacidad (1.0 activo, 0.35 inactivo, 0.6 idle). Active brand además marca punto 4px bajo el logo. Slot por marca `SizedBox(80×32, child: Center(...))` para uniformidad cross-brand. Used in AllProjects + AllNews (Opportunities dedicated screen was removed — ADR-52)
